...
async def run_pipeline(job_id: str, model_id: str, image_id: str,
    ...
    clerk_id     = image_record["clerk_id"]
    local_image  = f"/tmp/{job_id}_input.tif"
    cog_path     = f"users/{clerk_id}/cogs/{image_id}.tif"
    ...
    if aoi_geojson:
        ...
        try:
            ...
            log.warning("GCS download failed for vector %s (%s): %s — falling back to DB geometry",
                        vector_id, vector_record["gcs_path"], gcs_err)
            import json as _json
            geojson_fc = db.get_vector_geojson(vector_id)
            geojson_path = f"/tmp/{job_id}_aoi_db.geojson"
            with open(geojson_path, "w") as _f:
                _json.dump(geojson_fc, _f)
            aoi_shp = geo_utils.geojson_to_shp(geojson_path, job_id)
        local_image = geo_utils.clip_raster_to_vector(local_image, aoi_shp, job_id)
        log.info("Clipped image to vector AOI: %s", local_image)

    # ── 4. Phase 1: tile inference ────────────────────────────────────────────
    if db.is_cancelled(job_id):
        log.info("Job %s was cancelled before inference", job_id)
        return
    ...
    def progress_cb(done, total):
        ...
        milestone = (pct // 5) * 5
        if milestone > _last_pct_written[0]:
            _last_pct_written[0] = milestone
            db.update_job_status(job_id, "running", f"[1/2] Inference — {milestone}%")
    ...
    if db.is_cancelled(job_id):
        log.info("Job %s was cancelled before phase 2", job_id)
        return
    runner = RUNNERS.get(pipeline_type)
    if not runner:
        raise ValueError(f"Unknown pipeline_type: {pipeline_type}")

    db.update_job_status(job_id, "running", "[2/2] Counting — 0%")
    ...
    if "progress_callback" in _runner_params:
        _last_phase2_pct = [-1]
        def _phase2_cb(done, total):
            pct = int(done / total * 100)
            milestone = (pct // 10) * 10
            if milestone > _last_phase2_pct[0]:
                _last_phase2_pct[0] = milestone
                db.update_job_status(job_id, "running", f"[2/2] Counting — {milestone}%")
        _run_kwargs["progress_callback"] = _phase2_cb
    ...
    import os as _os
    for _path in [local_image, prob_raster]:
        try:
            _os.remove(_path)
        except OSError:
            pass
