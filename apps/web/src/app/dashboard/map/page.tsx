...
// MapLibre Draw (AOI polygon drawing)
...
// ── Area calculation (Shoelace on geographic coords, rough km²) ───────────────

function calcAreaKm2(coords: number[][]): number {
  // Approximate planar area in km² using Shoelace + degree-to-km conversion
  const R = 6371
  let area = 0
  ...
  const km2 = Math.abs(area / 2) *
    (Math.PI / 180) * R *
    (Math.PI / 180) * R * Math.cos((latMid * Math.PI) / 180)
...
export default function MapPage() {
  ...
  const [featurePopup, setFeaturePopup] = useState<{ props: FeatureProps; lngLat: maplibregl.LngLat } | null>(null)
  ...
  useEffect(() => {
    ...
    const handleClick = async (e: maplibregl.MapMouseEvent) => {
      ...
      // Query all visible vector layers
      const visibleVectorLayers = layers.filter(l => l.visible && l.type === 'vector')
      if (visibleVectorLayers.length === 0) return
      ...
      for (const layer of visibleVectorLayers) {
        ...
        if (props && Object.keys(props).length > 0) {
          setFeaturePopup({ props, lngLat: e.lngLat })
  ...
  // ── Cursor: pointer over vector layers ─────────────────────────────────────
  useEffect(() => {
    if (!map.current || !mapReady) return
    const enter = () => { if (!drawMode) map.current!.getCanvas().style.cursor = 'pointer' }
    const leave = () => { map.current!.getCanvas().style.cursor = '' }
    layers.filter(l => l.visible && l.type === 'vector').forEach(l => {
      const id = `layer-${l.id}`
      if (map.current!.getLayer(id)) {
        map.current!.on('mouseenter', id, enter)
        map.current!.on('mouseleave', id, leave)
      }
    })
    return () => {
      layers.forEach(l => {
  ...
  function copyAoi() {
    if (!aoi) return
    navigator.clipboard.writeText(JSON.stringify(aoi.geometry, null, 2))
  }
        ...
        <div className="px-5 py-3 border-t border-gray-100">
          ...
          {drawMode && (
            <p className="text-xs text-gray-400 mt-2 text-center">
              Click on map to place vertices. Double-click to finish.
            </p>
          )}
        </div>
