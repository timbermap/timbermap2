    ...
    <div className="w-full">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                {['Filename', 'EPSG', 'Geometry', 'Area (ha)', 'Size', 'Uploaded', 'Status', 'Actions'].map(h => (
                  <th key={h} className="text-left px-5 py-3 text-xs font-medium tracking-widest uppercase text-gray-400">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[...vectors].sort((a,b)=>{
                    ...
                    if(av>bv) return sortDir==='asc'?1:-1
                    return 0
                  }).slice((page-1)*PAGE_SIZE,page*PAGE_SIZE).map(v => {
                const isActive = activeVectorIds.has(v.id)
                return (
                  <tr key={v.id} className="border-b border-gray-50 hover:bg-[#F4F9F9] transition-colors">
                    <td className="px-5 py-3.5 text-gray-900 font-medium max-w-[180px]">
                      <span className="truncate block">{v.filename}</span>
                    </td>
                    <td className="px-5 py-3.5 text-gray-500 font-mono text-xs">{v.epsg || '—'}</td>
                    ...
                    <td className="px-5 py-3.5 text-gray-400 text-xs whitespace-nowrap">{formatDate(v.created_at)}</td>
          ...
          {/* Pagination */}
          {vectors.length > PAGE_SIZE && (
            <div className="flex items-center justify-between mt-4 px-1">
              <p className="text-xs text-gray-400">{vectors.length} vectors · page {page} of {Math.ceil(vectors.length/PAGE_SIZE)}</p>
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setPage(p => Math.min(Math.ceil(vectors.length/PAGE_SIZE),p+1))} disabled={page===Math.ceil(vectors.length/PAGE_SIZE)}
