...
export default function VectorsPage() {
  ...
  useEffect(() => {
    ...
    const toFetch = vectors.filter(v => v.status === 'ready' && !fetchedPreviewIds.current.has(v.id))
    if (!toFetch.length) return
    toFetch.forEach(v => fetchedPreviewIds.current.add(v.id))
    Promise.all(
      toFetch.map(v =>
        fetch(`${API}/vectors/${v.id}/preview?clerk_id=${user.id}`)
          .then(async res => {
  ...
  async function uploadSingle(item: UploadItem, index: number) {
    ...
    const upd = (patch: Partial<UploadItem>) =>
  ...
  async function handleDeleteAll() {
    ...
    setDeletingAll(true)
    try {
      const deletable = vectors.filter(v => !activeVectorIds.has(v.id))
      await Promise.all(deletable.map(v =>
  ...
  const sorted = [...vectors].sort((a, b) => {
    let av: string | number = a[sortKey as keyof Vector] as string | number || ''
    let bv: string | number = b[sortKey as keyof Vector] as string | number || ''
    ...
    <div className="w-full">
      ...
      {confirmAll && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-xl">
            ...
            {(() => {
              const skipped   = vectors.filter(v => activeVectorIds.has(v.id)).length
              const deletable = vectors.length - skipped
              return (
                <div className="text-center mb-6">
                  <p className="text-sm text-gray-500">
                    This will permanently delete <span className="font-medium text-gray-700">{deletable} vector{deletable !== 1 ? 's' : ''}</span> and all their files from storage and the database.
                  </p>
                  {skipped > 0 && (
                    <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2 mt-3">
                      {skipped} vector{skipped !== 1 ? 's' : ''} with active jobs will be skipped.
                    </p>
                  )}
                </div>
              )
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleDeleteAll} disabled={deletingAll || vectors.filter(v => !activeVectorIds.has(v.id)).length === 0}
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <button onClick={() => fileRef.current?.click()}
          className="inline-flex items-center gap-2 bg-[#3D7A72] text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-[#2A5750] transition-colors shadow-sm cursor-pointer">
          <UploadIcon />Upload shapefile
        </button>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-100">
            ...
            <div className="flex gap-2 mt-6">
              <button onClick={handleTransform} disabled={transforming || !newEpsg}
                className="flex-1 bg-[#3D7A72] text-white py-2.5 rounded-xl text-sm font-medium hover:bg-[#2A5750] transition-colors disabled:opacity-50 shadow-sm cursor-pointer disabled:cursor-default">
                {transforming ? 'Queuing...' : 'Run transform'}
              </button>
              <button onClick={() => { setShowTransform(false); setSelectedId(null) }}
                className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm hover:bg-gray-50 transition-colors cursor-pointer">
                Cancel
              </button>
            </div>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-100">
            <h2 className="text-base font-semibold text-[#1C1C1C] mb-1">Delete vector</h2>
            ...
            <div className="flex gap-2">
              <button onClick={handleDelete} disabled={isDeleting}
                className="flex-1 bg-rose-400 text-white py-2.5 rounded-xl text-sm font-medium hover:bg-rose-500 transition-colors disabled:opacity-50 cursor-pointer disabled:cursor-default">
      ...
      {!isLoaded || loading ? (
        ...
        <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-20 text-center cursor-pointer hover:border-[#A0CECC] hover:bg-[#F4F9F9] transition-all group"
          ...
          <div className="w-12 h-12 rounded-2xl bg-[#EEF7F6] flex items-center justify-center mx-auto mb-4 text-[#6AA8A0]">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6"><path fillRule="evenodd" d="M8.157 2.176a1.5 1.5 0 0 0-1.147 0l-4.084 1.69A1.5 1.5 0 0 0 2 5.25v10.877a1.5 1.5 0 0 0 2.074 1.386l3.51-1.452 4.26 1.762a1.5 1.5 0 0 0 1.146 0l4.083-1.69A1.5 1.5 0 0 0 18 14.75V3.872a1.5 1.5 0 0 0-2.073-1.386l-3.51 1.452-4.26-1.762ZM7.5 5v11.25L5 17.5V5.75l2.5-1.034V5Zm2.5 11.25V5L12.5 3.75v11.25L10 16.25Zm2.5.5v-11.5l2.5-1.034V14.25l-2.5 1.034v-.5.466Z" clipRule="evenodd"/></svg>
          </div>
                    ...
                    return (
                      <tr key={v.id} className="border-b border-gray-50 hover:bg-[#F4F9F9] transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-900 font-medium max-w-[180px]">
                          <span className="truncate block text-xs md:text-sm" title={v.filename}>{v.filename}</span>
                        </td>
                        <td className="px-4 py-3 text-gray-500 font-mono text-xs">{v.epsg || '—'}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs">{v.geometry_type || '—'}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs">{v.area_ha ? v.area_ha.toLocaleString() : '—'}</td>
