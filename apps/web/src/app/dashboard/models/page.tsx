...
export default function ModelsPage() {
  ...
  // Models the user can actually use (has access + visible)
  const activeModels = allModels.filter(m => m.has_access && m.is_visible)
  const proModels  = activeModels.filter(m => !m.is_free)
  const freeModels = activeModels.filter(m => m.is_free)
  const isPro = allModels.some(m => !m.is_free && m.has_access)

  const headerBtn = (
    <a href="/dashboard/catalog"
      className="flex items-center gap-2 bg-[#EEF7F6] hover:bg-[#D6EEED] text-[#3D7A72] font-medium text-sm px-3.5 py-2 rounded-xl transition-colors border border-[#A0CECC]/50 cursor-pointer">
      <CatalogIcon />
      Model catalog
    </a>
  )
